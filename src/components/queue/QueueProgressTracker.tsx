...
function StepLine({ done, delay }: { done: boolean; delay: number }) {
  ...
  useEffect(() => {
    progress.value = withDelay(delay, withTiming(done ? 1 : 0, { duration: 500, easing: Easing.out(Easing.cubic) }));
  }, [done]);

  const lineStyle = useAnimatedStyle(() => ({
    backgroundColor: progress.value > 0
      ? `rgba(0, 78, 152, ${progress.value})`
      : '#E5E7EB',
  }));
...
function StepDot({ done, icon: IconComp, label, isLast, delay }: { done: boolean; icon: any; label: string; isLast: boolean; delay: number }) {
  ...
  useEffect(() => {
    scale.value = done
      ? withDelay(delay, withSpring(1, { damping: 10, stiffness: 150 }))
  ...
  return (
    <Animated.View entering={FadeIn.duration(300).delay(delay)} style={{ alignItems: 'center', flex: 1 }}>
      <Animated.View
        style={[
          dotStyle,
          {
            ...
            backgroundColor: done ? '#004E98' : '#F3F4F6',
      ...
      <Text style={{ fontSize: 11, textAlign: 'center', color: done ? '#004E98' : '#9CA3AF', fontWeight: done ? '600' : '400', fontFamily: 'Inter-Medium' }}>
        {label}
      </Text>
